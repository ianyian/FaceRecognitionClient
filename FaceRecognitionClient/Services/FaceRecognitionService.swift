...
class FaceRecognitionService {
    ...
    func matchFaceAgainstLocalCache(_ detectedEncoding: MediaPipeFaceEncoding) -> FaceMatchingResult {
        ...
        let hasAllLandmarks = detectedEncoding.allLandmarks != nil && (detectedEncoding.allLandmarks?.count ?? 0) >= FaceRecognitionService.MIN_ALL_LANDMARKS
        let landmarkMode = hasAllLandmarks ? "478-landmark" : "33-landmark"
        
        print("🔍 Matching against \(mediaPipeRecords.count) faces using \(landmarkMode) mode with threshold \(String(format: "%.0f%%", FaceRecognitionService.CONFIDENCE_THRESHOLD * 100))")
        
        // Debug: Show detected encoding info
        print("📸 Camera encoding: \(detectedEncoding.landmarks.count) key landmarks, \(detectedEncoding.allLandmarks?.count ?? 0) total landmarks")
        ...
        for faceData in mediaPipeRecords {
            ...
            if isFirstRecord {
                print("💾 Stored encoding: \(storedEncoding.landmarks.count) key landmarks, \(storedEncoding.allLandmarks?.count ?? 0) total landmarks")
                isFirstRecord = false
            }
            ...
            if let allLandmarkSimilarity = calculateAllLandmarksSimilarity(detectedEncoding, storedEncoding) {
                ...
                if matchedCount < FaceRecognitionService.MIN_MATCHING_LANDMARKS {
                    print("   📊 \(faceData.studentName): \(String(format: "%.1f%%", similarity * 100)) (\(matchType) lm, \(matchedCount) matched - SKIP: below min)")
                    continue
                }
            }
            
            print("   📊 \(faceData.studentName): \(String(format: "%.1f%%", similarity * 100)) (\(matchType) landmarks)")
        ...
        if let result = votingResult, result.finalScore >= effectiveThreshold {
            ...
            print("✅ Match found: \(match.studentName) with \(match.similarityPercentage) similarity (threshold: \(String(format: "%.0f%%", effectiveThreshold * 100)))")
            return FaceMatchingResult(match: match, bestSimilarity: result.finalScore, bestCandidateName: result.faceData.studentName, candidatesChecked: mediaPipeRecords.count)
        } else {
            let bestScore = votingResult?.finalScore ?? 0
            let bestName = votingResult?.faceData.studentName
            print("❌ No match above threshold. Best: \(String(format: "%.1f%%", bestScore * 100)) (\(bestName ?? "none"))")
            return FaceMatchingResult(match: nil, bestSimilarity: bestScore, bestCandidateName: bestName, candidatesChecked: mediaPipeRecords.count)
        }
    }
    ...
    private func applyMultiSampleVoting(_ studentScores: [String: [(similarity: Double, faceData: FaceDataDocument)]]) -> (finalScore: Double, faceData: FaceDataDocument, voteCount: Int, avgScore: Double)? {
        
        var bestResult: (finalScore: Double, faceData: FaceDataDocument, voteCount: Int, avgScore: Double)?
        
        for (studentId, scores) in studentScores {
            ...
            if sampleCount >= 3 {
                ...
                if votesAboveThreshold >= 3 {
                    ...
                    print("🗳️ Voting [\(bestFaceData.studentName)]: \(sampleCount) samples, \(votesAboveThreshold) votes, boost: +\(String(format: "%.1f%%", votingBoost * 100))")
                ...
                if sampleCount >= 3 {
                    let scoreRange = sortedScores[0].similarity - sortedScores[min(2, sampleCount - 1)].similarity
                    if scoreRange < 0.05 {
                        // Very consistent scores (within 5%)
                        let consistencyBonus = 0.01
                        finalScore = min(finalScore + consistencyBonus, 0.98)
                        print("   📊 Consistency bonus: +1% (range: \(String(format: "%.1f%%", scoreRange * 100)))")
                    }
                }
            }
            ...
            if bestResult == nil || finalScore > bestResult!.finalScore {
                bestResult = (finalScore: finalScore, faceData: bestFaceData, voteCount: votesAboveThreshold, avgScore: avgTopScore)
            }
        }
        
        // Log final voting result
        if let result = bestResult {
            print("🏆 Voting winner: \(result.faceData.studentName) - Final: \(String(format: "%.1f%%", result.finalScore * 100)), Votes: \(result.voteCount), AvgTop3: \(String(format: "%.1f%%", result.avgScore * 100))")
        }
    ...
    private func calculateMediaPipeSimilarity(_ encoding1: MediaPipeFaceEncoding, _ encoding2: MediaPipeFaceEncoding) -> Double {
        ...
        print("📊 Similarity: \(String(format: "%.1f%%", similarity * 100)) | Matched: \(matchingLandmarks) landmarks | Quality: \(String(format: "%.2f", qualityScore)) | AvgDist: \(String(format: "%.3f", averageWeightedDistance))")
    ...
    private func countMatchingLandmarks(_ encoding1: MediaPipeFaceEncoding, _ encoding2: MediaPipeFaceEncoding) -> Int {
    ...
    private func calculateAllLandmarksSimilarity(_ detected: MediaPipeFaceEncoding, _ stored: MediaPipeFaceEncoding) -> Double? {
        guard let detectedAll = detected.allLandmarks,
              ...
              storedAll.count >= FaceRecognitionService.MIN_ALL_LANDMARKS else {
        ...
        print("📊 All-478 Similarity: \(String(format: "%.1f%%", combinedSimilarity * 100)) | Matched: \(matchCount) landmarks | AvgDist: \(String(format: "%.4f", averageDistance)) | WeightedAvg: \(String(format: "%.4f", averageWeightedDistance))")
    ...
    private func calculateFacialRatioSimilarity(_ landmarks1: [MediaPipePoint3D], _ landmarks2: [MediaPipePoint3D]) -> Double {
    ...
    private func normalizeAllLandmarksImproved(_ landmarks: [MediaPipePoint3D]) -> [MediaPipePoint3D] {
