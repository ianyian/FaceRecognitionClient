...
struct StudentFormData {
    ...
    var isValid: Bool {
        studentFirstName.count >= 2 &&
        studentLastName.count >= 2 &&
        !className.isEmpty &&
        parentFirstName.count >= 2 &&
        parentLastName.count >= 2 &&
        parentPhone.count >= 10
    }
...
class StudentViewModel: ObservableObject {
    ...
    private func createStudent() async throws {
        ...
        let faceEncodingMarker = faceEncodings.isEmpty 
    ...
    private func updateStudent() async throws {
        guard let studentId = selectedStudent?.id else {
            throw NSError(domain: "Student", code: 400, userInfo: [NSLocalizedDescriptionKey: "No student selected"])
        }
    ...
    private func saveFaceData(
        ...
        for (index, encoding) in encodings.enumerated() {
            ...
            guard let encodingData = try? encoder.encode(encoding),
                  let encodingString = String(data: encodingData, encoding: .utf8) else {
            ...
            if index < capturedImages.count {
                let resizedImage = resizeImageForFirestore(image: capturedImages[index], maxSizeKB: 300)
