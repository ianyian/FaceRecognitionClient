...
struct StudentFormView: View {
    ...
    private var faceSamplesSection: some View {
        Section {
            // Status Row
            HStack {
                Image(systemName: viewModel.hasEnoughSamples ? "checkmark.circle.fill" : "exclamationmark.triangle.fill")
                    .foregroundColor(viewModel.hasEnoughSamples ? .green : .orange)
                
                VStack(alignment: .leading, spacing: 2) {
                    Text(viewModel.hasEnoughSamples ? "Face Samples Ready" : "Face Samples Required")
                        .font(.headline)
                    Text("\(viewModel.captureCount) of \(StudentViewModel.minFaceSamples)-\(StudentViewModel.maxFaceSamples) samples captured")
                        .font(.caption)
                ...
                CircularProgressView(
                    progress: Double(viewModel.captureCount) / Double(StudentViewModel.maxFaceSamples),
                    lineWidth: 4
                )
            ...
            if isEdit {
                Text("You can optionally capture new photos to replace the existing ones. Leave empty to keep current photos.")
            } else {
                Text("Capture 3-5 clear photos of the student's face from different angles for accurate recognition.")
            }
        }
    }
            ...
            if showCamera, let previewLayer = cameraService.getPreviewLayer() {
                ...
                CameraPreviewView(previewLayer: previewLayer)
                    ...
                    .overlay(
                        ...
                        VStack {
                            Spacer()
                            Text("👆 Tap to capture (\(viewModel.captureCount)/\(StudentViewModel.maxFaceSamples))")
                                .font(.subheadline)
                                .fontWeight(.medium)
            ...
            if showCamera {
                ...
                HStack(spacing: 12) {
                    ...
                    Button {
                        ...
                        .padding(.vertical, 12)
                        .background(viewModel.hasEnoughSamples ? Color.green : Color.gray.opacity(0.3))
    ...
    private var validationStatusSection: some View {
        Section {
            VStack(alignment: .leading, spacing: 12) {
                validationRow(
                    title: "Student Name",
                    isValid: viewModel.formData.studentFirstName.count >= 2 && viewModel.formData.studentLastName.count >= 2,
                    icon: "person.fill"
                )
                ...
                validationRow(
                    ...
                    isValid: viewModel.formData.parentFirstName.count >= 2 && viewModel.formData.parentLastName.count >= 2,
                    icon: "person.2.fill"
                )
                ...
                validationRow(
                    title: "Face Samples (\(viewModel.captureCount)/\(StudentViewModel.minFaceSamples)+)",
